 ...
 *
 * ## 2026-09-25:两个页签改名 + 新增「发现」
 *
 * 原来叫「内容」的那一格指向 `/create` —— 那是**创建表单**,不是内容管理,
 * 而真正的内容管理在 `/dashboard`,那一格却叫「收款」。**名与实正好错开**,
 * 用户点「内容」想找自己的东西,拿到的是新建表单。
 *
 * 现在按**它实际做的事**命名:
 * - 「内容」→ **「创建」**(它就是个表单)
 * - 「收款」→ **「内容与收款」**(`/dashboard` 两件事都做)
 *
 * ⭐ 新增的「**发现**」指向 `/explore`,它同时补上了一个更早就该有的东西 ——
 * 方案 §14.1 规定落地页要有三个入口,其中「**我是买家**」那一条
 * **从来没有被建过**(见 `docs/产品流程.md` §5.1)。在那之前,
