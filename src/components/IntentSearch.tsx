 ...
 *
 * 模型不可用时,下面那三个筛选项**照样在那儿**,只是不再自动填 —— 用户自己填。
...
export function IntentSearch({ items }: { items: readonly CatalogEntry[] }) {
  ...
  return (
    <>
      ...
      {s.notice?.kind === 'degraded' && (
        <Notice>
          没能解析
          {s.degradedQuestion !== null ? `「${s.degradedQuestion}」` : '刚才那句'} ——
          智能解析暂时不可用,已经换成手动筛选:
          <span className="text-neutral-300">请在下面分别填关键词和价格</span>。
        </Notice>
      )}

      {s.notice?.kind === 'failed' && (
        <Notice>
          没能问上(网络或服务端没应答)。再点一次「问一问」试试,也可以直接手动筛选。
          <button
            type="button"
            onClick={s.useManual}
            className="ml-2 rounded-lg border border-line bg-surface-2 px-3 py-1.5 text-[11px] text-neutral-200 transition-colors hover:border-accent"
          >
            直接手动筛选
          </button>
        </Notice>
...
/** 一条提示条。⚠️ 两种提示**互斥**(能重试的不降级,见 `useIntentSearch`) */
function Notice({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-5 rounded-2xl border border-accent/35 bg-accent/[0.07] px-5 py-4">
      <p className="text-xs leading-relaxed text-neutral-100">{children}</p>
    </div>
  )
}
