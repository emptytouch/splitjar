...
function step0() {
  ...
  if (!pk) {
    die(
      ...
      `  按方案 §6.2,它只该存在于你当前这个 shell,不进 .env、不进仓库、不进前端产物。\n` +
...
async function step1(ctx) {
  step(1, '挑一件「我买过的」内容 —— 买没买以链上为准,不看服务端怎么说')
  ...
  if (owned.length === 0) {
    die(
      ...
      `  · 先跑 \`node scripts/agent-buy.mjs\` 买一件(那一步要花钱),再回来跑这个脚本\n` +
...
/* ── 第 8 步:负对照 —— 签名合法但没买 ──────────────────────────────── */
step(WAIT_EXPIRY ? 6 : 5, '负对照:一个从没买过的地址签一份合法消息')
{
