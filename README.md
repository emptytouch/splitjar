...
- [x] **未付款 → 被拒**(2026-09-25 线上实测)。⚠️ **实际返回的是 `402 not_purchased`,
      **不是**本节下面与 `spec §16.1` 写的 401** —— 这是**故意的**:
      401 留给验签/时效/nonce,402 留给"请求成立但该付钱"。完整依据见
      `docs/W5-实施计划.md` §九.5
- [ ] ⚠️ **已付款 → 真的下载到文件**(比 `keccak256`:链上 `contentHash` 是 keccak256,
      所以按它比,**不是**本节下面写的 `sha256`)/ **URL 过期后失效**
      —— **不是"验了没通过",是本机发不出请求**(Blob 数据面与 `*.vercel.app`
      都是定向屏蔽)。**部署后必须补,这是 W5 的完成定义。**
      验法:`scripts/verify-unlock.mjs`(不花钱,只用已买过的那件)
