...
- [x] **`server/env.ts`** 是服务端变量的唯一事实来源 —— 把方案 §6.2「私钥不进前端
      产物」从"靠人记得"变成**编译期可拦**
- [x] **未付款 → 被拒**(2026-09-25 线上实测)。⚠️ **实际返回的是 `402 not_purchased`,
      **不是** `spec §16.1` 与 `docs/开发计划.md` §W5 写的 401** —— 这是**故意的**:
      401 留给验签/时效/nonce,402 留给"请求成立但该付钱"。完整依据见
      `docs/W5-实施计划.md` §九.5
- [x] **已付款 → 真的下载到文件 + URL 过期后失效**(2026-09-25 线上实测,6/6 全过)。
      `scripts/verify-unlock.mjs` 跑出来的三个数:下载 **277343 字节**;
      `keccak256` = 链上 `contentHash` = `0x5178052e…`;同一个 URL **63 秒后 403**。
      ⚠️ 比的是 **`keccak256`** —— 链上 `contentHash` 就是这个原语
      (`W5-实施计划.md` §473 写的 `sha256` 是早期措辞,验收时以链上那个值为准)
...
- [x] **反例矩阵线上 14/0/3**（2026-09-25,详见 `docs/W8-实施计划.md` §6.6）
      —— 第 ② 条已修复（为 other 单取报价）。⑩ 下架内容 / ⑦ 生产 HMAC / ④ burn
      三条仍验不了,是环境与资源限制,不是缺陷
...
**W8 Agent 演示客户端** —— 已完成(2026-09-24 落地,09-25 线上实测)。
...
- [x] `npm run typecheck` + `npm run build` 都过;本机实测(含**负对照**)见
      `docs/W8-实施计划.md` §六
- [x] **第 5–8 步(真花钱那段)线上跑通**(2026-09-25,见 `docs/W8-实施计划.md` §6.6)
      —— 链上 5 笔付款,含 agent 自己 approve+pay+取内容;第 7 条「人类+agent 并排」
      舞台已齐,`[Agent]` 徽章验证通过
